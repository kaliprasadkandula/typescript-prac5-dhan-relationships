...
const company_router = (0, express_1.default)();
//post the products 
company_router.post('/products/', async (req, resp) => {
    const rb = req.body;
    const company_repo = await AppDataSource.getRepository(companyEntity_1.Company);
    const product_repo = await AppDataSource.getRepository(productEntity_1.Product);
    let company_entity = await company_repo.findOne({ where: { name: rb.company } });
    let comp_id;
    const prod = new productEntity_1.Product();
    prod.name = rb.name;
...
company_router.get('/products/:company', async (req, resp) => {
    const company_repo = await AppDataSource.getRepository(companyEntity_1.Company);
    const product_repo = await AppDataSource.getRepository(productEntity_1.Product);
    let company_entity = await company_repo.findOne({ where: { name: req.params.company } });
    if (!company_entity) {
        resp.status(404).send({ error: 'Company not found' });
    }
